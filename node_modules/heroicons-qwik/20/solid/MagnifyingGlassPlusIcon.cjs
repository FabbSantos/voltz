"use strict";
const qwik = require("@builder.io/qwik");
const jsxRuntime = require("@builder.io/qwik/jsx-runtime");
const MagnifyingGlassPlusIcon = /* @__PURE__ */ qwik.componentQrl(qwik.inlinedQrl(({ title, titleId, ...props }) => {
  return /* @__PURE__ */ jsxRuntime.jsxs("svg", {
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 20 20",
    fill: "currentColor",
    "aria-hidden": "true",
    "aria-labelledby": titleId,
    ...props,
    children: [
      title ? /* @__PURE__ */ jsxRuntime.jsx("title", {
        id: titleId,
        children: title
      }) : null,
      /* @__PURE__ */ jsxRuntime.jsx("path", {
        d: "M9 6a.75.75 0 01.75.75v1.5h1.5a.75.75 0 010 1.5h-1.5v1.5a.75.75 0 01-1.5 0v-1.5h-1.5a.75.75 0 010-1.5h1.5v-1.5A.75.75 0 019 6z"
      }),
      /* @__PURE__ */ jsxRuntime.jsx("path", {
        fillRule: "evenodd",
        d: "M2 9a7 7 0 1112.452 4.391l3.328 3.329a.75.75 0 11-1.06 1.06l-3.329-3.328A7 7 0 012 9zm7-5.5a5.5 5.5 0 100 11 5.5 5.5 0 000-11z",
        clipRule: "evenodd"
      })
    ],
    [qwik._IMMUTABLE]: {
      children: false
    }
  });
}, "MagnifyingGlassPlusIcon_component_Wv9trSQ9rQM"));
module.exports = MagnifyingGlassPlusIcon;
